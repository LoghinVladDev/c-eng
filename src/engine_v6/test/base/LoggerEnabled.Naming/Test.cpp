...
TEST(LoggerEnabledNaming, Anonymous) {
    ...
    ASSERT_TRUE(outbuf1.str().contains("logger = anonymous_logger"));
    ASSERT_TRUE(outbuf2.str().contains("logger = anonymous_logger"));
    ASSERT_NE(outbuf1.str(), outbuf2.str());
    ASSERT_TRUE(outbuf1.str().contains("test"));
    ASSERT_TRUE(outbuf2.str().contains("other"));
}
...
TEST(LoggerEnabledNamic, Named) {
    ...
    ASSERT_TRUE(outbuf1.str().contains("one test"));
    ASSERT_FALSE(outbuf1.str().contains("another test"));
    ASSERT_TRUE(outbuf1.str().contains("yet one more"));
    ASSERT_TRUE(outbuf1.str().contains("final test"));

    ASSERT_FALSE(outbuf2.str().contains("one test"));
    ASSERT_TRUE(outbuf2.str().contains("another test"));
