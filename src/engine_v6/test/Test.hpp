...
namespace testing {
    ...
    protected:
        auto _assert(cds::String const& reason, std::source_location const& location = std::source_location::current()) const noexcept(false) -> void {
    ...
    auto Test::start() const noexcept -> bool {
        ...
        try {
            run();
            std::cout << "\033[1;32m" "Test " << _instance << " " << _name << " ran successfully\n";
        } catch (TestAssert const & assert) {
            std::cout << "\033[1;31m" "Test " << _instance << " " << _name << " failed\n";
