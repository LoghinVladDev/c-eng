...
//    shader->setVec3("light.direction", glm1::vec3(-0.2f, -1.0f, -0.3f));
    ...
    shader->setVec3("light.position", camera->getTransform().getLocation());
    shader->setVec3("light.direction", camera->getFront());

    shader->setFloat("light.cutoff", glm::cos(glm::radians(16.5f)));
...
//    glm1::mat4 lightModel = glm1::scale(glm1::translate(glm1::mat4(1.0f), lightLoc), lightScale);
...
//    lightLoc = glm1::vec3(sin(glm1::radians(offset)) * 1.5, sin(glm1::radians(offset)) * 0.66  , cos(glm1::radians(offset)) * 1.5);
//
    glBindVertexArray(lightVAO);
//
    glDrawArrays(GL_TRIANGLES, 0, 36);
